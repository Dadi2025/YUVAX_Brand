    ...
    <>
      ...
      <nav className={`navbar ${isScrolled ? 'scrolled' : ''}`}>
        <div className="container navbar-container">
          ...
          {/* Desktop Menu */}
          <div className="hidden md:flex items-center gap-8 nav-links-center">
            <Link to="/" className="nav-link">Home</Link>
            <Link to="/shop" className="nav-link">Collections</Link>
            <Link to="/style-wall" className="nav-link">Style Wall</Link>

            {/* Dashboard Links */}
            {user && <Link to="/profile" className="nav-link" style={{ color: 'var(--accent-primary)' }}>📦 Orders</Link>}
            {isAgentLoggedIn && <Link to="/delivery/dashboard" className="nav-link" style={{ color: 'var(--accent-primary)' }}>📊 Dashboard</Link>}
            {isAdminLoggedIn && <Link to="/admin/dashboard" className="nav-link" style={{ color: 'var(--accent-primary)' }}>📊 Dashboard</Link>}
          </div>
          ...
          {/* Mobile Menu Button */}
          <button className="md:hidden text-black bg-transparent border-none text-2xl cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
            {isOpen ? '✕' : '☰'}
          </button>
        </div>
